				...
				'fields' => array(
					array(
						...
						'html'  => '<code>' . esc_html( rest_url( Alpha_Ortto_Account_SF_ID_Updater::REST_NAMESPACE . Alpha_Ortto_Account_SF_ID_Updater::REST_ROUTE ) ) . '</code>'
							. '<p class="description">Account journeys don\'t have a "dynamic" webhook action, so this can\'t write the result straight back onto the Account like a Person journey webhook can. Instead, add a (classic) Webhook action to an Account journey: method POST, URL as above, header <code>X-Api-Key</code> set to the secret above, and exactly one payload field with key name <code>' . esc_html( Alpha_Ortto_Account_SF_ID_Updater::PAYLOAD_KEY ) . '</code> mapped to the Account\'s 15 character Salesforce ID field -- use that exact key name, not <code>id</code> or <code>account_id</code>, which collide with keys already reserved elsewhere in Ortto\'s webhook payload. This endpoint converts the ID and calls Ortto\'s Accounts API directly to write the 18 character result onto the matching Account -- both field ids below must already exist on the Account object (Settings -> Customer data -> Fields), and must be the actual Ortto field id (e.g. str:oib:... or str:cm:...), not the payload\'s key name. The 18 character field is only ever written if it\'s currently empty on that Account -- an existing value is never overwritten.</p>',
					),
					array(
